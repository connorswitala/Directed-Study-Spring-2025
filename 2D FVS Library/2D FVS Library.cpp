...
Matrix A_Minus(const Vector& U, const Point& normals) {

	double nx = normals.x;
	double ny = normals.y;

	Vector V = constoPrim(U);  
	double rho = V[0];
	double u = V[1];
	double v = V[2];
	double p = V[3];
...
// Function that calculates A+
Matrix A_Plus(const Vector& U, const Point& normals) {
	double nx = normals.x;
	double ny = normals.y;
...
//		
//		for (int i = 1; i < Nx - 1; ++i) {
//			pair<Matrix1D, double> interim = solveMiddleLine(U, U_inlet, dU_old, grid, top, bottom, i, Nx, Ny, dt); 
//			dU_new.line(i) = interim.first;
//			inner_residual = inner_residual + interim.second;  
//		}	
//	
...
//		swap(dU_old, dU_new);  
//		cout << "Inner Residual = " << inner_residual << endl; 
//	}
//
//	return dU_old; 
//
...
//pair<Matrix1D, double> solveMiddleLine(Matrix2D& U, Matrix U_inlet, Matrix2D& dU_old, const Grid& grid, 
...
//			);
//
//		res = res + F[0][0] / grid.Volume(i, j) * F[0][0] / grid.Volume(i, j);
//
...
//
//
//Matrix2D calculateJacobians(Matrix2D& Fluxes, Matrix2D& U, Matrix& U_inlet, const Grid& grid, int Nx, int Ny, 
//	BoundaryCondition leftBoundary, BoundaryCondition rightBoundary, BoundaryCondition bottomBoundary, BoundaryCondition topBoundary) {
//
//	Duo leftBC, rightBC, bottomBC, topBC;   
//
//	for (int j = 0; j <= Ny; ++j) {
//
//		leftBC = Boundary2D(leftBoundary, U.vars(0,j), U_inlet, grid.LNorms(0, j));
//
//		Fluxes.vars(0, j) = A_Plus(U.vars(0, j), grid.LNorms(0, j)) + leftBC.second*(A_Minus())
//	}
//
//}